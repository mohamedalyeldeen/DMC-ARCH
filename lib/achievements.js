...
// schedule shifts, sick-leave pushes, reschedule-remaining); the legacy
// `due` field is only updated on a direct manual edit through the modal,
// so it silently drifts stale whenever a task's dates move for any other
// reason. That drift is what made these disagree with the Board's own
// on-time badge, which already compares against endDate correctly.
function isOnTime(t) { return !t.endDate || (!!t.completedAt && t.completedAt <= t.endDate); }
function isFinishedEarly(t) { return !!(t.endDate && t.completedAt && t.completedAt < t.endDate); }
function isCurrentlyOverdue(t, todayStr) { return t.status !== 'done' && !!t.endDate && t.endDate < todayStr; }
