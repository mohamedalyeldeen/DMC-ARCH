...
// ---------- WORKING WEEK (Friday & Saturday off) + PUBLIC HOLIDAYS ----------

// Owner-managed via the Holidays sheet tab (date, name). Set once per
// request by server.js before any date math runs, via setHolidays() below —
// every scheduling function routes through isNonWorkingDay(), so a holiday
// is automatically respected everywhere the weekend already was: auto-
// scheduling, shifting, overlap checks, capacity, and the start/end date
// validation on manual entry.
let holidaySet = new Set();
function setHolidays(dates) {
  holidaySet = new Set((dates || []).filter(Boolean));
}
...
function isNonWorkingDay(dateStr) {
  ...
  return dow === 5 || dow === 6 || holidaySet.has(dateStr);
}
...
// Shifts an engineer's still-open (not done) tasks forward by `workDays`
// working days, starting at (and including) `fromSequence` — leaves
// anything before that sequence untouched. Used by both sick-leave logging
// (fromSequence 0, i.e. the whole queue) and the "reschedule remaining"
// action on an overdue task (fromSequence = that task's own sequence, so
// only it and whatever comes after it for that engineer moves).
function pushEngineerQueueFrom(tasks, assigneeId, fromSequence, workDays) {
  if (!workDays) return;
  engineerTasks(tasks, assigneeId)
    .filter(t => t.status !== 'done' && (t.sequence || 0) >= fromSequence)
    .forEach(t => {
      t.startDate = shiftByWorkingDays(t.startDate, workDays);
      t.endDate = shiftByWorkingDays(t.endDate, workDays);
    });
}

// Shifts EVERY one of an engineer's still-open (not done) tasks forward by
// `workDays` working days — used when logging sick leave: the whole
// remaining queue moves out by however many working days they're out,
// regardless of which specific tasks happened to be "in progress" during
// the leave itself.
function pushEngineerQueueForLeave(tasks, assigneeId, workDays) {
  pushEngineerQueueFrom(tasks, assigneeId, 0, workDays);
}

module.exports = {
  todayStr, addDays, diffDaysInclusive, rangesOverlap,
  isNonWorkingDay, setHolidays, nextWorkingDay, workingDaysInclusive, addWorkingDays, shiftByWorkingDays,
  engineerTasks, detectOverlap, nextSequence,
  insertWithShift, removeAndCompact, restoreRemovedTask, nextAvailableDate,
  pushEngineerQueueForLeave, pushEngineerQueueFrom,
